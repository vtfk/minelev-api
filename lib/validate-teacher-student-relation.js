...
module.exports = async (caller, student) => {
  if (!student) throwNoAccess(student)

  // Retreive all students
  const students = await getMyStudents(caller)

  // If a student was specified, verify that the teacher has access to this student before proceeding
  const specifiedStudent = students.filter(s => s.userName === student || s.email === student)
