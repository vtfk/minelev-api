...
const replaceShortcodes = (elevforhold) => {
  let kortnavn = elevforhold.skole.organisasjon.kortnavn.startsWith('OF-') ? elevforhold.skole.organisasjon.kortnavn.replace('OF-', '') : elevforhold.skole.organisasjon.kortnavn // Make it match pifu data
  kortnavn = kortnavn.startsWith('OPT-') ? kortnavn.replace('OPT-', '') : kortnavn // Make it match pifu data
  kortnavn = kortnavn.startsWith('UT-') ? kortnavn.replace('UT-', '') : kortnavn // Make it match pifu data
  return kortnavn
}
  ...
  */
  ...
  for (const elevforhold of bothElevforhold) {
    const repackedFag = elevforhold.faggruppemedlemskap.map(medlemskap => {
      ...
      const kortnavn = replaceShortcodes(elevforhold)
