const { verify } = require('azure-jwt-verify')
const { TOKEN_AUTH, DEMO, DEMO_USER } = require('../config')
const HTTPError = require('./http-error')

module.exports = async (context, request, next) => {
  if (DEMO) {
    request.token = { upn: DEMO_USER, DEMO }
    return next(context, request)
  }

  const bearerToken = request.headers.authorization
  if (!bearerToken) {
    context.log.warn('warn', ['with-token-auth', request.url, 'no-authorization-header'])
    return new HTTPError(400, 'Authorization header is missing').toJSON()
  }

  try {
    const token = bearerToken.replace('Bearer ', '')
    const validatedToken = await verify(token, TOKEN_AUTH)
    request.token = JSON.parse(validatedToken).message

    return next(context, request)
  } catch (error) {
    context.log.error(['with-token-auth', request.url, 'invalid-token', error])
    return new HTTPError(401, 'Authorization token is invalid').toJSON()
  }
}
